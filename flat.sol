...
// File: gelatoHysteria.sol

contract hysteriaReader is Ownable {
    ...
    function debtTrigger(address strategy) public view returns(bool trigger){
        if (isInactive(strategy)){
            return (false);
        }
        uint256 debtRatio = IStrat(strategy).calcDebtRatio();
        trigger = (debtRatio > (IStrat(strategy).debtUpper().add(hysteriaDebt)) || debtRatio < IStrat(strategy).debtLower().sub(hysteriaDebt));
    }
    
    function collatTrigger(address strategy) public view returns(bool trigger ){
        if (isInactive(strategy)){
            return (false);
        }
        uint256 collatRatio = IStrat(strategy).calcCollateral();
        trigger = (collatRatio > IStrat(strategy).collatUpper().add(hysteriaCollateral) || collatRatio < IStrat(strategy).collatLower().sub(hysteriaCollateral));
    }
